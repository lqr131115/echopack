...
class Service {
  ...
  start = async () => {
    ...
    await this.emitHooks(C.BEFORE_HOOKS_RESISTER);
    await this.registerHooks();
    await this.emitHooks(C.AFTER_HOOKS_RESISTER);
      ...
      compiler = webpack(webpackConfig, (error, stats) => {
        if (error) {
          ...
          if (errors.length) {
            ...
            log.info(
              "COMPILE SUCCESS!",
              time >= 0 ? `compile finish in ${time / 1000} s` : ""
            );
          }
        }
      });
